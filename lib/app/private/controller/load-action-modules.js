...
module.exports = function loadActionModules (sails, cb) {
  ...
  // Try loading controllers from pre-generated bundle first (Fase 3)
  var manifestCache = sails.config.moduleManifestCache;
  var bundlePath = require('path').resolve(sails.config.appPath, '.tmp', 'build', 'controllers-bundle.js');
  if (manifestCache && require('fs').existsSync(bundlePath)) {
    try {
      var bundleFiles = require(bundlePath);
      ...
      Object.keys(bundleFiles).forEach(function(globalId) {
        var moduleDef = bundleFiles[globalId];
        var filePath = globalId;

        // Handle directory path dots → slashes
        if (require('path').dirname(filePath) !== '.') {
          filePath = require('path').dirname(filePath).replace(/\./g, '/') + '/' + require('path').basename(filePath);
        }
      ...
      // (Shallow) merge moduleDefinitions overrides
      _.each(_.get(sails, 'config.controllers.moduleDefinitions') || {}, function(action, actionIdentity) {
        helpRegisterAction(sails, action, actionIdentity, true);
      });

      console.log('⏱ loadActionModules (BUNDLE):', (Date.now() - _t) + 'ms (' + Object.keys(bundleFiles).length + ' controllers)');
      return cb();
    } catch (e) {
      sails.log.verbose('Failed to load controllers bundle, falling back to includeAll...', e);
    }
  }
  ...
  includeAll.optional({
    ...
    console.log('⏱ loadActionModules (SCAN):', (Date.now() - _t) + 'ms (' + _.keys(sails._actions).length + ' actions registered)');
