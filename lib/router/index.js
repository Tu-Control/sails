...
Router.prototype.flush = function(routes) {
  ...
  console.log('⏱ router.flush (explicit routes):', (Date.now() - _tFlush) + 'ms (' + sortedRouteAddresses.length + ' explicit routes)');

  var _tShadow = Date.now();

  // Fired after static routes are bound → triggers bindShadowRoutes in blueprints hook
  sails.emit('router:after');

  console.log('⏱ router:after (shadow routes):', (Date.now() - _tShadow) + 'ms');
};
