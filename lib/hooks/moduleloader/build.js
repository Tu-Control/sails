...
module.exports = function sailsBuild(appPath) {
  ...
  function scanDir(dirname, filter) {
    filter = filter || /\.js$/;
    if (!fs.existsSync(dirname)) return {};
    var results = {};

    function recurse(currentDir, relativePath) {
  ...
  // 1. Models
  // includeAll lowercases model identities (caseSensitive: false by default)
  // and applies replaceExpr /^.*\// to strip directory prefixes.
  // We must match this behavior so the manifest keys match what Sails/ORM expects.
  var modelsDir = path.join(appPath, 'api', 'models');
  var modelsRaw = scanDir(modelsDir);
  var models = {};
  Object.keys(modelsRaw).forEach(function(key) {
    // Apply replaceExpr /^.*\// (strip directory prefix) + lowercase
    var transformedKey = key.replace(/^.*\//, '').toLowerCase();
    models[transformedKey] = modelsRaw[key];
  });
  ...
  var services = scanDir(servicesDir);
  ...
  try {
    ...
    // All node_modules are external (we only inline app code)
    var externals = Object.keys(appPkg.dependencies || {})
      .concat(Object.keys(appPkg.devDependencies || {}))
      .concat(['sails', 'path', 'fs', 'util', 'events', 'crypto', 'os',
               'http', 'https', 'stream', 'zlib', 'url', 'querystring',
