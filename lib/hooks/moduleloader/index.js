module.exports = function(sails) {
  ...
  var manifestCache = require('./manifest-cache');
  ...
  return {
    ...
    initialize: function(cb) {
      console.log('-------------------------------------------------------');
      console.log('🛠️  [Sails Fork] Running from local source code.');
      console.log('-------------------------------------------------------');
...
/**
     ...
     *
     * Loading strategy (3-tier, same as services):
     * 1. BUNDLE — pre-built .tmp/build/models-bundle.js (single require, fastest)
     * 2. CACHE  — manifest with file paths (184 requires, avoids readdir/stat)
     * 3. SCAN   — include-all full directory scan (fallback)
     *
     * @param {Object} options
     * @param {Function} cb
     */
    loadModels: function (cb) {
      ...
      // Tier 1: Try pre-built bundle (single require replaces 184 individual requires)
      var bundlePath = path.resolve(sails.config.appPath, '.tmp', 'build', 'models-bundle.js');
      if (sails.config.moduleManifestCache && fs.existsSync(bundlePath)) {
        try {
          var models = require(bundlePath);

          // Set globalId for each model (matches include-all behavior where
          // globalId = PascalCase filename, e.g. AiUsageAudit from AiUsageAudit.js)
          // Derive from manifest file paths which preserve original casing.
          var manifestForGlobalId = path.resolve(sails.config.appPath, '.tmp', 'build', 'module-manifest.json');
          if (!fs.existsSync(manifestForGlobalId)) {
            manifestForGlobalId = path.resolve(sails.config.appPath, '.tmp', 'module-manifest.json');
          }
          if (fs.existsSync(manifestForGlobalId)) {
            try {
              var manifestData = JSON.parse(fs.readFileSync(manifestForGlobalId, 'utf8'));
              if (manifestData.sections && manifestData.sections.models && manifestData.sections.models.files) {
                var filesMap = manifestData.sections.models.files;
                for (var identity in models) {
                  if (!models[identity].globalId && filesMap[identity]) {
                    models[identity].globalId = path.basename(filesMap[identity], path.extname(filesMap[identity]));
                  }
                }
              }
            } catch (manifestErr) {
              // If manifest read fails, infer globalId from identity (first-letter uppercase,
              // which handles simple cases like "banco" → "Banco").
              // Complex cases like "aiusageaudit" → "AiUsageAudit" need the manifest.
              for (var identity2 in models) {
                if (!models[identity2].globalId) {
                  models[identity2].globalId = identity2.charAt(0).toUpperCase() + identity2.slice(1);
                }
              }
            }
          } else {
            // No manifest at all — infer simple globalId from identity
            for (var identity3 in models) {
              if (!models[identity3].globalId) {
                models[identity3].globalId = identity3.charAt(0).toUpperCase() + identity3.slice(1);
              }
            }
          }

          console.log('⏱ loadModels (BUNDLE):', (Date.now() - _t) + 'ms', '(' + _.keys(models).length + ' models)');
          return cb(undefined, models);
        } catch (e) {
          sails.log.verbose('Failed to load models bundle, falling back to manifest/scan...', e);
        }
      }

      // Tier 2: Prefer the build-generated manifest (has all sections + buildGenerated:true)
      var manifestPath = path.resolve(sails.config.appPath, '.tmp', 'build', 'module-manifest.json');
      if (!fs.existsSync(manifestPath)) {
        manifestPath = path.resolve(sails.config.appPath, '.tmp', 'module-manifest.json');
      }
      ...
      // Try loading from manifest cache first
      if (sails.config.moduleManifestCache) {
        var cached = manifestCache.loadFromManifest(manifestPath, 'models', modelsOptions);
        if (cached !== null) {
          console.log('⏱ loadModels (CACHE):', (Date.now() - _t) + 'ms', '(' + _.keys(cached).length + ' models)');
          return cb(undefined, cached);
        }
      }

      // Tier 3: Fallback — scan normal
      includeAll.optional(modelsOptions, function(err, models) {
        if (err) { return cb(err); }
        console.log('⏱ loadModels (SCAN):', (Date.now() - _t) + 'ms', '(' + _.keys(models).length + ' models)');

        // Save to manifest for next startup
        if (sails.config.moduleManifestCache) {
          manifestCache.saveToManifest(manifestPath, 'models', modelsOptions, models);
        }
    ...
    loadServices: function (cb) {
      ...
      // Try loading from pre-generated bundle first (Fase 2)
      var bundlePath = path.resolve(sails.config.appPath, '.tmp', 'build', 'services-bundle.js');
      if (sails.config.moduleManifestCache && fs.existsSync(bundlePath)) {
        try {
          var services = require(bundlePath);
          console.log('⏱ loadServices (BUNDLE):', (Date.now() - _t) + 'ms', '(' + _.keys(services).length + ' services)');
          return bindToSails(cb)(undefined, services);
        } catch (e) {
          sails.log.verbose('Failed to load services bundle, falling back to manifest/scan...', e);
        }
      }

      // Prefer the build-generated manifest (has all sections + buildGenerated:true)
      var manifestPath = path.resolve(sails.config.appPath, '.tmp', 'build', 'module-manifest.json');
      if (!fs.existsSync(manifestPath)) {
        manifestPath = path.resolve(sails.config.appPath, '.tmp', 'module-manifest.json');
      }
      ...
      // Try loading from manifest cache first
      if (sails.config.moduleManifestCache) {
        var cached = manifestCache.loadFromManifest(manifestPath, 'services', servicesOptions);
        if (cached !== null) {
          console.log('⏱ loadServices (CACHE):', (Date.now() - _t) + 'ms', '(' + _.keys(cached).length + ' services)');
          return bindToSails(cb)(undefined, cached);
        }
      }

      // Fallback: scan normal
      includeAll.optional(servicesOptions, bindToSails(function(err, services) {
        if (err) { return cb(err); }
        console.log('⏱ loadServices (SCAN):', (Date.now() - _t) + 'ms', '(' + _.keys(services).length + ' services)');

        // Save to manifest for next startup
        if (sails.config.moduleManifestCache) {
          manifestCache.saveToManifest(manifestPath, 'services', servicesOptions, services);
        }
    ...
    loadUserHooks: function (cb) {
      ...
      async.auto({
        ...
        nodeModulesFolder: function(cb) {
          var nodeModulesDir = path.resolve(sails.config.appPath, 'node_modules');
          var manifestPath = path.resolve(sails.config.appPath, '.tmp', 'module-manifest.json');
          // Also check build directory for manifest
          if (!fs.existsSync(manifestPath)) {
            manifestPath = path.resolve(sails.config.appPath, '.tmp', 'build', 'module-manifest.json');
          }

          // Try loading cached installed hooks (the processed result, not raw scan)
          if (sails.config.moduleManifestCache) {
            var cachedHooks = manifestCache.loadFromManifest(manifestPath, 'installedHooks', {
              dirname: nodeModulesDir
            });
            if (cachedHooks !== null) {
              console.log('⏱ nodeModulesFolder (CACHE): skipped scan, loaded', Object.keys(cachedHooks).length, 'installed hooks');
              return cb(undefined, cachedHooks);
            }
          }
        ...
        if (err) {return cb(err);}
        console.log('⏱ loadUserHooks:', (Date.now() - _t) + 'ms', '(scan complete)');

        // Save installedHooks to manifest (only if we just scanned - not if we loaded from cache)
        if (sails.config.moduleManifestCache && results.nodeModulesFolder) {
          var manifestPathForHooks = path.resolve(sails.config.appPath, '.tmp', 'module-manifest.json');
          var nodeModulesDir = path.resolve(sails.config.appPath, 'node_modules');
          manifestCache.saveToManifest(manifestPathForHooks, 'installedHooks', {
            dirname: nodeModulesDir
    ...
    loadBlueprints: function (cb) {
      ...
      includeAll.optional({
        ...
        console.log('⏱ loadBlueprints:', (Date.now() - _t) + 'ms', '(' + _.keys(blueprints).length + ' blueprints)');
        return cb(err, blueprints);
      });
    },
    ...
    loadResponses: function (cb) {
      ...
      if (sails.config.moduleManifestCache) {
        var cached = manifestCache.loadFromManifest(manifestPath, 'responses', responsesOptions);
        if (cached !== null) {
          console.log('⏱ loadResponses (CACHE):', (Date.now() - _t) + 'ms', '(' + _.keys(cached).length + ' responses)');
          return bindToSails(cb)(undefined, cached);
        }
      }

      includeAll.optional(responsesOptions, bindToSails(function(err, responses) {
        if (err) { return cb(err); }
        console.log('⏱ loadResponses (SCAN):', (Date.now() - _t) + 'ms', '(' + _.keys(responses).length + ' responses)');

        if (sails.config.moduleManifestCache) {
          manifestCache.saveToManifest(manifestPath, 'responses', responsesOptions, responses);
        }
