  ...
  loadFromManifest: function(manifestPath, sectionName, options) {
    try {
      ...
      for (var identity in section.files) {
        ...
        // Set globalId from original filename (like includeAll does).
        // e.g. api/models/AiUsageAudit.js → globalId = 'AiUsageAudit'
        if (!modules[identity].globalId) {
          modules[identity].globalId = path.basename(filePath, path.extname(filePath));
        }
      }
